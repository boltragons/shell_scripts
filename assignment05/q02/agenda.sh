...
# date: 22/09/2023
# Descrition: Manages a database file that has persons names and emails by CRUD operations.
...
${usage_text}
-h			Help.
-a <name> <email>	Adds a person with name and email.
-l			Lists all persons.
-r <email>		Removes the person with the email.
-u <email> <name>	Updates the name of the person with the given email.  
EOF
)
...
-u)
	...
	updated_user=$(grep -E ":$2$" ${db} | cut -f1 -d':')
	sed -i "s/^.*:$2$/$3:$2/" ${db}
	echo "Updated user \"${updated_user}\" to \"$3\"."
	;;
-h)
	echo "${help_text}"
	;;
